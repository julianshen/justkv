...
- [ ] **Step 4: Add a library target so integration tests can import the crate**
...
- [ ] **Step 5: Implement the compiled format**
...
- [ ] **Step 6: Implement load dispatch**
...
- [ ] **Step 6: Wire up `serve` in main.rs**
...
- [ ] **Step 2: Add dev-dependencies needed by the tests**
...
- [ ] **Step 1: Create the example dataset and dockerignore**
...
- [ ] **Step 5: Verify the validation gate actually fails the build**
...
- [ ] **Step 2: Generate the dataset and measure**
...
- [ ] `cargo test` — all unit, property, CLI and integration tests pass
- [ ] `cargo clippy --all-targets` — no warnings indicating real defects
- [ ] `./scripts/pack.sh --data data/example.tsv --tag justkv:test` succeeds
- [ ] `./scripts/pack.sh --data /tmp/dup.tsv` fails with a duplicate-key message
- [ ] `docker run --rm -p 8080:8080 justkv:test` serves `/kv/alpha`, `/health`, `/metrics`, `/stats`
- [ ] `bench/README.md` contains measured numbers, no `<placeholders>`
- [ ] A default-served response returns 200 with `X-JustKV-Default: 1` and increments `justkv_misses_total`
