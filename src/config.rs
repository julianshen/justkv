...
#[command(
    name = "justkv",
    about = "Read-only in-memory key-value server",
    version
)]
pub struct Cli {
    #[command(subcommand)]
...
fn parse_ascii_delimiter(s: &str) -> Result<char, String> {
    ...
    if chars.next().is_some() {
        return Err(format!(
            "delimiter must be exactly one character, got {s:?}"
        ));
    }
    if !c.is_ascii() {
...
mod tests {
    ...
    fn healthcheck_defaults_to_serve_bind_default() {
        ...
        let Command::Healthcheck(a) = cli.command else {
